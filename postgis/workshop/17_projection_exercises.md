...
![서경 74도 자오선에 중간 정점을 추가한 뒤 평면 좌표계로 변환하여 곡선 경로와 도로 교차를 보존하는 과정](projection/projection-geography-bridge.png)

*그림 17-1. 두 끝점만 변환하면 투영 후에도 직선으로 연결되지만, `ST_Segmentize`로 자오선에 정점을 충분히 추가한 뒤 `ST_Transform`하면 평면 좌표계에서의 곡률을 보존할 수 있습니다. 아래의 평면선과 대권 호 비교는 다음 장의 `geography` 개념으로 이어집니다.*
