...
![Broad St 주변의 가까운 도로 탐색과 모든 지하철역별 최근접 도로 LATERAL 조인 흐름](knn/nearest-neighbor-flow.png)

*그림 29-1. 왼쪽은 Broad St 포인트에서 공간 인덱스를 따라 가까운 도로 후보만 좁혀 상위 3개를 반환하는 과정입니다. 오른쪽은 각 지하철역 행마다 `LATERAL` 내부 쿼리를 실행해 가장 가까운 도로 한 개를 연결하는 과정입니다. 지도와 인덱스 트리는 검색 원리를 설명하는 개념도입니다.*
