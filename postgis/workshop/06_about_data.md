...
이 워크숍의 데이터는 뉴욕시의 셰이프파일 4개와 사회인구학적 변수의 속성 테이블 1개입니다. 우리는 Shapefile을 PostGIS 테이블로 로드했으며 나중에 워크숍에서 사회인구통계학적 데이터를 추가할 예정입니다.
...
![뉴욕시 도로망](screenshots/nyc_streets.png)
...
> `nyc_homicides`는 로컬 확장 데이터이며, 공식 `About our data` 페이지에는 대응 이미지가 없습니다.
