...
> [!NOTE]
> Esri의 선형 참조 용어에서는 선형 공간 피처를 담은 기본 테이블과 "이벤트"를 담은 비공간 테이블을 구분합니다. 이벤트 테이블에는 공간 피처를 참조하는 외래 키와 해당 피처를 따라 측정한 위치값이 들어 있습니다. 이 장에서도 이러한 비공간 테이블을 "이벤트 테이블"이라고 부릅니다.
