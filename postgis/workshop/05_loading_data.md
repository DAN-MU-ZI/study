...
PostGIS 워크숍 데이터 디렉터리의 `2000/` 하위 디렉터리에는 2010년 자료로 대체된 2000년 인구조사 셰이프파일이 있습니다. 이 백업 파일로 데이터 로드를 연습하면 이미 불러온 데이터와의 이름 충돌을 피할 수 있습니다. 다음 지침을 실행할 때는 셸의 현재 디렉터리를 `2000/`으로 변경하세요.
...
셰이프파일은 정확히 무엇일까요? 일반적으로 공통 파일명(예: nyc_census_blocks)을 쓰는 `.shp`, `.shx`, `.dbf` 및 기타 확장자 파일의 묶음을 셰이프파일이라고 합니다. 핵심 데이터는 `.shp` 파일에 들어 있지만, `.shp` 파일만으로는 필수 보조 파일이 빠져 있어 온전하게 배포할 수 없습니다.
