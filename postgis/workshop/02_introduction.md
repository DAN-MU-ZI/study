...
**공간 데이터베이스는 데이터베이스의 다른 개체와 마찬가지로 공간 개체를 저장하고 조작합니다.**
...
**진정한 공간 데이터베이스는 사람들이 공간 특징을 최고 수준의 데이터베이스 개체로 취급하기 시작하면서 탄생했습니다.**
...
인덱스가 유용하려면 빠르게 수행되어야 합니다. 따라서 B-트리처럼 정확한 결과를 제공하는 대신 공간 인덱스는 대략적인 결과를 제공합니다. "이 다각형 안에는 어떤 선이 있습니까?"라는 질문이 있습니다. 대신 공간 인덱스에 의해 "이 다각형의 경계 상자 안에 포함된 경계 상자가 있는 선은 무엇입니까?"로 해석됩니다.
...
PostgreSQL은 강력한 관계형 데이터베이스 관리 시스템(RDBMS)입니다. 이는 BSD 스타일 라이센스로 출시되므로 무료이며 오픈 소스 소프트웨어입니다. 다른 많은 오픈 소스 프로그램과 마찬가지로 PostgreSQL은 단일 회사에 의해 제어되지 않고 [글로벌 개발자 커뮤니티](https://www.postgresql.org/community/contributors/)와 이를 개발할 회사가 있습니다.
...
MySQL 공간객체는 특수한 경우로 스트링타입 위에서 해킹을 해야 했기 때문에 MySQL 코드는 전체 코드 베이스에 분산되어 있었습니다. PostGIS 0.1 개발에는 한 달도 채 걸리지 않았습니다. "MyGIS" 0.1을 수행하는 것은 훨씬 더 오랜 시간이 걸렸기 때문에 결코 빛을 보지 못했을 것입니다.
...
2001년 5월, [Refractions Research](http://www.refractions.net/)에서 PostGIS의 첫 번째 버전이 출시되었습니다. PostGIS 0.1에는 객체, 인덱스 및 몇 가지 기능이 있었습니다. 그 결과 저장과 검색에는 적합했지만 분석에는 적합하지 않은 데이터베이스가 탄생했습니다.
...
PostGIS 데이터 용량이 증가함에 따라 또 다른 문제가 표면화되었습니다. 즉, 지오메트리를 저장하는 데 사용되는 표현이 상대적으로 비효율적인 것으로 나타났습니다. 점이나 짧은 선과 같은 작은 개체의 경우 표현의 메타데이터에 최대 300%의 오버헤드가 발생했습니다. 성능상의 이유로 표현에 다이어트가 필요했습니다. 메타데이터 헤더와 필수 차원을 줄임으로써 오버헤드가 크게 감소했습니다. PostGIS 1.0에서는 이 새롭고 더 빠르고 가벼운 표현이 기본값이 되었습니다.
