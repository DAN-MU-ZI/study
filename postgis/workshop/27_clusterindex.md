...
시스템 RAM은 최신 컴퓨터에서 가장 빠른 메모리는 아닙니다. 시스템 RAM과 실제 CPU 사이에는 여러 수준의 캐시가 있으며 기본 운영 체제와 프로세서는 블록 단위로 캐시 계층 구조를 위아래로 데이터를 이동합니다. 위로 이동하는 블록에 시스템이 다음에 필요로 할 데이터 조각이 포함되어 있다면... 그것은 큰 승리입니다. 메모리 구조와 공간 구조를 연관시키는 것은 승리할 확률을 높이는 방법입니다.
