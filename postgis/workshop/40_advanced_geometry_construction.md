...
- **first_stops**는 각 개별 그룹의 첫 번째 레코드만 가져와 **stops_distance** 출력을 필터링합니다. 우리가 **stops_distance**를 주문한 방식으로 인해 첫 번째 레코드가 가장 먼 레코드입니다. 이는 각 지하철 노선을 구축하기 위한 시작 시드로 사용하려는 역임을 의미합니다.
