...
1. **pgAdmin**을 실행합니다.
   ...
   ![pgAdmin을 처음 실행한 서버 브라우저 화면](screenshots/pgadmin_01.png)
...
2. pgAdmin을 처음 실행한 경우 왼쪽 브라우저 트리 패널의 `Servers` 항목을 마우스 오른쪽 버튼으로 클릭하고 **Register > Server...**를 선택합니다.
   ...
   ![pgAdmin 서버 등록 대화상자의 General 탭](screenshots/pgadmin_02a.png)
...
2. `Databases`를 마우스 오른쪽 버튼으로 클릭하고 **Create > Database...**를 선택합니다.
   ...
   ![pgAdmin 서버 등록 대화상자의 Connection 탭](screenshots/pgadmin_02.png)
...
3. 아래와 같이 설정하고 **Save**를 클릭합니다.
   ...
   ![pgAdmin에서 새 데이터베이스 생성을 선택하는 메뉴](screenshots/pgadmin_03.png)

4. 생성된 `nyc` 데이터베이스 항목을 펼쳐 `Schemas > public` 구조를 확인합니다.

   ![nyc 데이터베이스 이름과 소유자를 지정하는 생성 대화상자](screenshots/pgadmin_04.png)

5. 상단 툴바의 **Query Tool** 아이콘을 클릭하거나 메뉴에서 *Tools > Query Tool*을 실행합니다.

   ![pgAdmin Query Tool에서 PostGIS 확장을 활성화하는 SQL](screenshots/pgadmin_05.png)
