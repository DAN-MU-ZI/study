...
앞 장까지는 토폴로지로 현재 공간 데이터의 구조적 무결성을 유지하는 방법을 배웠습니다. 이제 데이터가 시간에 따라 바뀌더라도 과거 상태를 잃지 않도록 **시간적 무결성**을 관리합니다.
...
![INSERT UPDATE DELETE 트리거가 현재 테이블과 TSTZRANGE 기반 이력 버전을 관리하고 과거 시점 뷰를 복원하는 흐름](history_tracking/trigger-history-timeline.png)

*그림 34-1. 트리거는 변경 전후의 버전을 이력 테이블에 남기고 각 행의 유효 기간을 `[시작, 종료)`로 기록합니다. 특정 시점 T를 포함하는 범위를 조회하면 그 시점의 공간 데이터를 복원할 수 있습니다.*
